...
public class InstantiationManager : MonoBehaviour
{
	...
	private void GetAssetCallback(PolyStatusOr<PolyAsset> getAssetResult, GameObject markerScene)
	{
		...
		PolyApi.Import(getAssetResult.Value, options, (asset, importResult) => ImportAssetCallback(asset, importResult, markerScene));
	}

	private void ImportAssetCallback(PolyAsset asset, PolyStatusOr<PolyImportResult> result, GameObject markerScene)
	{
		if (!result.Ok)
		{
		...
		polyModel.transform.parent = markerScene.transform;
		changeLayerToARBackground(polyModel.transform);
		polyModel.transform.position = new Vector3(0f, 0f, 0f);
		polyModel.transform.rotation = Quaternion.Euler(270, 0, 0);
	...
	private void addModel(GameObject markerScene, string resourceName)
	{
		...
		{
			PolyApi.GetAsset("assets/" + arAppResource._url, (getAssetResult) => GetAssetCallback(getAssetResult, markerScene));
		}
	...
	private ARAppResource getResource(string resourceName)
	{
		...
		// TODO:  Handle if resource doesn't exits
		return null;
	}
